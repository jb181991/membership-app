...
class UserController extends \TCG\Voyager\Http\Controllers\VoyagerBaseController
{
    ...
    public function index(Request $request)
    {
        ...
        if (strlen($dataType->model_name) != 0) {
            ...
            if ($dataType->scope && $dataType->scope != '' && method_exists($model, 'scope'.ucfirst($dataType->scope))) {
                ...
                // Customize retrieve of users
                if(Auth::user()->role_id == 4)
                {
                    $query = $model::select('*')->where(['role_id' => 5, 'coach_id' => Auth::user()->id]);
                } else {
                    $query = $model::select('*');
                }
            }
    ...
    public function edit(Request $request, $id)
    {
        ...
        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable', 'coaches', 'roles'));
    }
